  ...
  <v-container>
    <v-row class="wrapBox">
      <div class="welcomeBox col-2">
        ...
        <img src="../assets/images/welcome.png" alt="welcome" style="width:300px; margin-left:50px; margin-top : 100px;" />
      </div>
      <v-spacer></v-spacer>
      <div class="col-6 cambox">
        <div class="camInBox">
          ...
          <v-row class="camBottomBox">
            ...
            <v-spacer></v-spacer>
            <div @click="goCapture" class="clickBtn" @mouseover="hoverClickBtn" @mouseout="outClickBtn">
              <img v-if="!clickDialog" src="../assets/images/click.png" alt="클릭버튼" />
    ...
    goCapture() {
      ...
      http
        ...
        .then((response) => {
          ...
          if (response.data.isMask == 'NO MASK') alert(response.data.member.name + '님 마스크 안쓰면 뚝배기 날림');
          else alert(response.data.member.name + '님 반가워요');
        })
        .catch((error) => {
          if (error.response.data.detail == 'NO MASK') alert('등록 안한 고객님 마스크 안쓰시면 뚝배기 깹니다');
      ...
      Axios.post(uriBase + '?returnFaceId=true&returnFaceLandmarks=false&returnFaceAttributes=age,emotion', file, {
        ...
        .then((response) => {
          ...
          this.$store.commit('setEmotionAnalysis', response.data[0].faceAttributes.emotion);
          console.log();
        })
        .catch((error) => {
