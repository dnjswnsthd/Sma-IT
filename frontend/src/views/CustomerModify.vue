<template>
  <v-container class="col-8 centerContent">
    ...
    <DeleteDialog :dialogDelete="dialogDelete" :item="editedItem" @closeDelete="closeDelete"></DeleteDialog>
        ...
        .then((response) => {
          if (response.data.members.length >= this.limit) {
            ...
            for (var i = 0; i < this.limit; i++) {
              if (response.data.members.requirements == 'null') response.data.members.requirements = '없음';
              this.customers.push({
                customer_name: response.data.members[i].name,
                customer_age: response.data.members[i].age,
                customer_interest: response.data.members[i].interests,
                customer_require: response.data.members[i].requirements,
