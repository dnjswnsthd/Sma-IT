<template>
  <v-container class="col-6 centerContent">
    ...
    <div class="col-6 centerContent">
      ...
      <v-text-field v-model="member.interests" label="관심분야" type="string" dark></v-text-field>
      <v-text-field v-model="member.requirements" label="요구사항" type="string" dark></v-text-field>
      <v-row style="margin:0; padding:0;">
        <v-spacer></v-spacer>
    ...
    registInformation() {
      ...
      http
        ...
        .then((response) => {
          ...
          http
            .post(`/member/image/${this.imageName}`, formData)
            .then(() => {
              swal('등록 성공!', {
                icon: 'success',
              });
